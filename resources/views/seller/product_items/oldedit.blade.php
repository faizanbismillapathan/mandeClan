...
<div class="card-body">
     @if(!empty($record))
       {!! Form::model($record,array('url' => ['seller/product-items', $record->id],'method'=>'PATCH','class'=>'','id' =>'comman_form_id','files'=>'true')) !!}
  ...
  <div class="form-group col-md-4">
            ...
            <div class="form-group col-md-4">
                <label for="inputEmail4">Price : * (Price you entering is IN USD)</label>                                                                 {!!Form::text('item_price',null,array('class'=>'form-control numbersOnly numbersOnly','placeholder'=>'','autocomplete'=>'off','required')) !!} 
            </div>
...
<div class="col-md-2 padding0"><div class="checkbox"><label><input type="checkbox" values="{{$data1->unit_value}}" ids="{{$data1->id}}" name="attr_value_id[{{$data1->unit_value}}]" value="{{$data1->id}}" <?php if (in_array($data1->id, $checked_attributs)) echo 'checked="checked"'; ?>> {{$data1->unit_value}}</label></div></div>
@endforeach
</div>
