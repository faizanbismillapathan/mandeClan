...
@section('title',"Create New Subscriptions | service  Mande Clan")
 ...
 <div class="row">
    <div class="col-md-4">
      <div class="row">
        ...
        <div class="col-md-6">
         @if(!empty($inform->service_plan_name))
          <span>{{$inform->service_plan_name}}</span>
        ...
        <section class="pricing my-5 py-5 ">
            <div class="container-fluid">
                <div class="row">
                    ...
                    @foreach($service_plans as $index=>$data)
                <div class="mt-3 col-lg-4">
                <div class="card border rounded border-rounded mb-5">
@if( !empty($inform) && $inform->service_plan_name==$data->service_plan_name && $inform->plan_status=='Active')            
 ...
 <h1 class="display-5 text-center text-white bg-primary p-3"> <i class="fas fa-dollar-sign text-white"></i> {{$data->service_plan_price}} <span class="period"> / {{$data->service_plan_validity}} Days</span></h1>
                                    <hr>
                                    {!!$data->service_plan_features!!}
                   
</div>
                   @if(!empty($inform))                 
@if($inform->service_plan_name==$data->service_plan_name && $inform->plan_status=='Active')            
...
@else
<div class="card-footer" style="background-color : #ececece8">
  <button type="submit"  data-toggle="modal" data="{{$data->id}}" data-target="#defaultModalPrimary1" class="btn btn-block btn-primary text-uppercase purchase_plan_modal m-1" plan_id="{{$data->id}}" totalAmount="{{$data->service_plan_price}}">Subscribe   </button>
</div>
  @endif
...
<!--                                        {{$data->service_plan_name}} -->
<!--                                    </h5>-->
<!--                                    <h1 class="display-5 text-center"> <i class="fas fa-dollar-sign"></i> {{$data->service_plan_price}}<span class="period">/{{$data->service_plan_validity}} Days</span></h1>-->
<!--                                    <hr>-->
<!--                                    {!!$data->service_plan_features!!}-->
                   
<!--                   @if(!empty($inform))                 -->
<!--@if($inform->service_plan_name==$data->service_plan_name && $inform->plan_status=='Active')            -->
<!--<button type="button" class="btn btn-block btn-success text-uppercase"  data-toggle="modal" data="{{$data->id}}" data-target="#UnSubscribealertmesg">-->
<!--<i class="fa fa-check-circle"></i> UnSubscribe-->
<!--</button>-->
    
<!--    @else-->

<!--      <button type="submit"  data-toggle="modal" data="{{$data->id}}" data-target="#alertmesg" class="btn btn-block btn-primary text-uppercase">Subscribe   </button>          -->

<!--@endif-->
<!--@else-->

<!--  <button type="submit"  data-toggle="modal" data="{{$data->id}}" data-target="#defaultModalPrimary1" class="btn btn-block btn-primary text-uppercase purchase_plan_modal" plan_id="{{$data->id}}" totalAmount="{{$data->service_plan_price}}">Subscribe   </button>-->
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Are you sure want to UnSubscribe the @if(!empty($inform)){{$inform->service_plan_name}} @endif?</p>
                    </div>
        ...
        <div class="vertical-align-inner-div">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>First Unsubscribe current @if(!empty($inform)) {{$inform->service_plan_name}} @endif plan  For subscribe New plan !!</p>
                    </div>
                    <div class="modal-footer">
                        {{-- <button type="button" class="btn btn-raised btn-danger order-yes" id="modal_delete">Yes</button> --}}
...
{{Form::close()}}
  ...
  <div class="tab-pane fade show " id="id_tab_paypal" role="tabpanel" aria-labelledby="tab_paypal">
                ...
                {!!Form::open(['url'=>['service/paypal_transection'],'method' =>'post', 'role'=>'form','class' =>'require-validationsss form-bordered form-row-stripped','id'=>'comman_form_id','data-cc-on-file'=>'false'])!!}   
 ...
 @if (Session::has('success'))
              ...
              {!!Form::open(['url'=>['service/stripe_transection'],'method' =>'POST', 'role'=>'form','class' =>'require-validation form-bordered form-row-stripped','id'=>'comman_form_id','data-cc-on-file'=>'false','data-stripe-publishable-key'=>env('STRIPE_KEY')])!!}
...
// alert(id)
        ...
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                 ...
                 $.ajax({
          url: "{{url('service/unsubscribe_plan')}}",
          type: "post",
