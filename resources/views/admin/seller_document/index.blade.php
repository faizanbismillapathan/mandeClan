  ...
  <main class="content">
                    <div class="container-fluid p-0">
                        ...
                        <div class="clearfix">
                             ...
                             <a href="{{url('admin/seller-document/create')}}" class="form-inline float-right mt--1 d-none d-md-flex">
                                       <button class="btn btn-primary"><i class="fa fa-plus-circle"></i> Create a New Seller Document</button>
                                    </a>

                            <h1 class="h3 mb-3">Seller Document &nbsp;&nbsp;@if(!empty($records))({{count($records)}})@endif</h1>
                        </div>
                        ...
                        <div class="card">
                                <div class="card-body">
                                        <div class="row">
                                            <div class="col-md">
                                            {{Form::open(['url'=>['admin/seller-document'],'method'=>'GET'])}}
                                            
@if(!empty($seller_documents))
                                                {!!Form::select('search',$seller_documents,null,array('class'=>'form-control select2','placeholder'=>'Search','data-toggle'=>'select2','onchange'=>'this.form.submit()')) !!}
@endif
...
<td>{{$data->category_name}}</td>
  <td style="color: #e91e63;text-decoration :underline; " data-toggle="tooltip" data-placement="top" title="Total Add / Add Document" class=" clickable-row" data-href="{{ URL::to('admin/seller-document/'.$data->id) }}" data-underline>{{$data->count}} / {{$data->total_document}}</td>
  <td style="color: green;text-decoration :underline; " data-toggle="tooltip" data-placement="top" title="Pending /Approved Document" class=" clickable-row" data-href="{{ URL::to('admin/seller-document/'.$data->id) }}" data-underline>{{$data->total_pending}} / {{$data->total_approved}}</td>
...
@if(!empty($data->document_file))
     <a href="{{asset('public/images/seller_document/'.$data->document_file)}}" target="_blank"> {{$data->document_file}}</a> @else
@endif --}}
</td>
...
<input type="checkbox" data-toggle="toggle" data-on="Active" data-off="Deactive" data-onstyle="warning" data-offstyle="success" id="{{$data->id}}" class="checkstatus_kyc" onchange="updateToggle1({{$data->id}})" >
   @endif
                                                     ...
                                                     {{--<td>                                                    
                                                <a href="{{ URL::to('admin/seller-document/'.$data->id.'/edit') }}"><button class="btn btn-info" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i></button></a>
