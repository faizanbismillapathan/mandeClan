...
@section('title',"Create New Subscriptions | customer  Mande Clan")
...
<div class="clearfix">
    {{-- <a href="{{url('customer/subscriptions')}}" class="form-inline float-right mt--1 d-none d-md-flex">
 ...
 <div class="row">
    <div class="col-md-4">
      <div class="row">
        ...
        <div class="col-md-6">
         @if(!empty($inform->customer_plan_name))
          <span>{{$inform->customer_plan_name}}</span>
        ...
        <section class="pricing py-5">
            <div class="container">
                <div class="row">
                    ...
                    @if(!empty($customer_plans))
                    @foreach($customer_plans as $index=>$data)
                                            <div class="mt-3 col-lg-4">
                            <div class="card mb-5 mb-lg-0">
                                <div class="card-body">
                                    <h5 class="card-title text-muted text-uppercase text-center">
                                        {{$data->customer_plan_name}} 
                                    </h5>
                                    <h1 class="display-5 text-center"> <i class="fas fa-dollar-sign"></i> {{$data->customer_plan_price}}<span class="period">/{{$data->customer_plan_validity}} Days</span></h1>
                                    <hr>
                                    {!!$data->customer_plan_features!!}
                   
                   @if(!empty($inform))                 
@if($inform->customer_plan_name==$data->customer_plan_name && $inform->plan_status=='Active')            
...
@else
  ...
  <button type="submit"  data-toggle="modal" data="{{$data->id}}" data-target="#defaultModalPrimary1" class="btn btn-block btn-primary text-uppercase purchase_plan_modal" plan_id="{{$data->id}}" totalAmount="{{$data->customer_plan_price}}">Subscribe   </button>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Are you sure want to UnSubscribe the @if(!empty($inform)){{$inform->customer_plan_name}} @endif?</p>
                    </div>
        ...
        <div class="vertical-align-inner-div">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>First Unsubscribe current @if(!empty($inform)) {{$inform->customer_plan_name}} @endif plan  For subscribe New plan !!</p>
                    </div>
                    <div class="modal-footer">
                        {{-- <button type="button" class="btn btn-raised btn-danger order-yes" id="modal_delete">Yes</button> --}}
...
{{Form::close()}}
  ...
  <div class="tab-pane fade show " id="id_tab_paypal" role="tabpanel" aria-labelledby="tab_paypal">
                ...
                {!!Form::open(['url'=>['customer/paypal_transection'],'method' =>'post', 'role'=>'form','class' =>'require-validationsss form-bordered form-row-stripped','id'=>'comman_form_id','data-cc-on-file'=>'false'])!!}   
 ...
 @if (Session::has('success'))
              ...
              {!!Form::open(['url'=>['customer/stripe_transection'],'method' =>'POST', 'role'=>'form','class' =>'require-validation form-bordered form-row-stripped','id'=>'comman_form_id','data-cc-on-file'=>'false','data-stripe-publishable-key'=>env('STRIPE_KEY')])!!}
...
// alert(id)
        ...
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                 ...
                 $.ajax({
          url: "{{url('customer/unsubscribe_plan')}}",
          type: "post",
